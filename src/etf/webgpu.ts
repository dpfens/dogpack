 ...
 * regardless of resolution or how many channels are involved.
 */
interface WebGPUResources {
   ...
   *  profiling in computeInternal(). Falls back gracefully to coarse
   *  CPU-side timing (submitAndGpuWait) when unavailable. */
...
export class WebGpuEdgeTangentFlowComputer extends BaseWebGPUStrategy implements ETFComputer {
  ...
  private async computeInternal(
    ...
    const channelInputBufs = inputs.map((channel) =>
      createBufferWithData(device, channel.data, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST)
    );
    ...
    if (queryReadBuf) {
      ...
      for (let i = 0; i < passLabels.length; i++) {
        ...
        if (label.startsWith('refine[')) label = 'refine (sum)';
        else if (label.startsWith('gradient[')) label = 'gradient (sum)';
        else if (label.startsWith('tensorAccumulate[')) label = 'tensorAccumulate (sum)';
    ...
    for (const buf of channelInputBufs) buf.destroy();
