 ...
 * none of this depends on image width/height/channel count, so it's
 * created once and reused for every compute()/computeMultiChannel() call
 * regardless of resolution or how many channels are involved. This
 * remains true with row-band tiling: pipelines only reference sizes
 * through the per-dispatch Params uniform, never at pipeline-creation
 * time.
 */
interface WebGPUResources {
  ...
  blurHTiledPipeline: GPUComputePipeline;
  blurVTiledPipeline: GPUComputePipeline;
  tangentExtractPipeline: GPUComputePipeline;
  tangentRefinePipeline: GPUComputePipeline;
  /** Whether the device supports 'timestamp-query'. Currently unused by
   *  computeInternal() (detailed per-pass GPU timestamp profiling was
   *  dropped when band-tiling/pipelining was added, since attributing
   *  per-pass GPU timestamps across overlapping in-flight bands adds a
   *  lot of bookkeeping for diagnostic-only value) — kept on the
   *  resources object in case coarse per-band timing is reintroduced. */
  hasTimestampQuery: boolean;
}

/**
 * One full set of band-sized GPU buffers. Two of these are allocated
 * (once) per computeInternal() call and reused round-robin across every
 * band, rather than allocated/destroyed per band — both to avoid
 * reallocation overhead and because it's what makes the double-buffered
 * pipelining in computeInternal() possible (see that function's header
 * comment).
 *
 * Every buffer here is sized for the *largest* band that will occur
 * (maxBandBufHeight rows); a shorter final band (image height not evenly
 * divisible by the band's core row count) simply uses a prefix of each
 * buffer.
 */
interface BandBufferSet {
  ...
  tangentBuf1: GPUBuffer;
  tangentBuf2: GPUBuffer;
  /** COPY_DST | MAP_READ; final tangent buffer is copied here for CPU
   *  readback at the end of each band. */
...
export class WebGpuEdgeTangentFlowComputer extends BaseWebGPUStrategy implements ETFComputer {
  ...
  private static async initResources(): Promise<WebGPUResources> {
    ...
    this.resourcesPromise = (async () => {
      ...
      const makePipeline = (
        code: string,
        entryPoint = 'main',
        constants: Record<string, number> = { WORKGROUP_SIZE }
      ): GPUComputePipeline =>
  ...
  private async computeInternal(
    ...
    const { bandRows, numBands } = planBandLayout(
      width,
      height,
      channelCount,
      halo,
      device.limits,
      WebGpuEdgeTangentFlowComputer.bandMemoryBudgetBytes
    );
    ...
    return this.runGuarded(device, async () => {
      ...
      const channelScratch: Float32Array[][] = [0, 1].map(() =>
        inputs.map(() => new Float32Array(width * maxBandBufHeight))
      );
      ...
      const pendingReadback: (Promise<void> | null)[] = [null, null];
      ...
      try {
        ...
        await Promise.all(pendingReadback.filter((p): p is Promise<void> => p !== null));
        ...
        for (const bufs of slots) destroyBandBufferSet(bufs);
        kernelBuf.destroy();
      }
...
function planBandLayout(
  ...
  if (bandBufHeight * width * 16 > maxBindableBytes) {
    throw new Error(
      `[EdgeTangentFlowWebGPU] Cannot fit even a ${MIN_BAND_ROWS}-row band ` +
 ...
 */
function createBandBufferSet(
  device: GPUDevice,
  width: number,
  maxBandBufHeight: number,
  channelCount: number
): BandBufferSet {
  const pixelCount = width * maxBandBufHeight;
  return {
    channelInputBufs: Array.from({ length: channelCount }, () =>
      device.createBuffer({
 ...
 */
function writeBandOutputRows(
  mapped: Float32Array,
  width: number,
  bandStartY: number,
  bandRows: number,
  halo: number,
  tangentsOut: Float32Array,
  magnitudeOut: Float32Array,
  anisotropyOut: Float32Array
): void {
