...
export class WebGPUGradientAlignedBlur implements BlurStrategy {
  ...
  private static cachedDevice: GPUDevice | null = null;
  private static deviceInitPromise: Promise<GPUDevice> | null = null;
  private static lastUnsupportedReason: string | undefined;
  private static errorListenerAttached = false;

  private flowTexture: GPUTexture | null = null;
  private flowFieldWidth = 0;
  private flowFieldHeight = 0;
  private flowDirty = true;
  private flowBakePromise: Promise<GPUTexture> | null = null;
  ...
  constructor(config: GradientAlignedBlurBackendConfig) {
    ...
    if (!device) {
      throw new Error(
        '[GradientAlignedBlur/WebGPU] No cached GPUDevice. isSupported() must resolve true before construction.',
      );
    }
    ...
    this.maxTileBytes = Math.max(
      WORKGROUP_SIZE * 4, // never go below one row's worth of data
      Math.floor(
   ...
   * the cache.
   */
  private static async acquireDevice(): Promise<GPUDevice> {
  ...
  private assertWithinTextureLimits(width: number, height: number): void {
    const maxDim = this.device.limits.maxTextureDimension2D;
    if (width > maxDim || height > maxDim) {
      throw new Error(
        `[GradientAlignedBlur/WebGPU] Image ${width}x${height} exceeds this device's ` +
   ...
   */
  private async getFlowTexture(width: number, height: number): Promise<GPUTexture> {
    if (
      this.flowTexture &&
      !this.flowDirty &&
      this.flowFieldWidth === width &&
  ...
  async blur(input: ChannelImage, sigma: number): Promise<ChannelImage> {
    ...
    if (wantedHalfSamples > MAX_SAMPLES - 1) {
      console.warn(
        `[GradientAlignedBlur/WebGPU] halfSamples clamped to ${MAX_SAMPLES - 1} (sigma=${sigma} wanted ${wantedHalfSamples}); kernel truncated. Raise MAX_SAMPLES if this matters.`,
      );
    }
