...
export class FDoG implements DoGImplementation {
  ...
  async process(input: ChannelImage, overrides: Partial<FDoGConfig> = {}): Promise<ChannelImage> {
    ...
    const p = weighting.pByMagnitude && typeof params.p === 'number' && confidence
      ? ScalarField.materialize(
          ScalarField.scale(ScalarField.constant(params.p), confidence!),
          input.width,
          input.height
        )
      : params.p;
    ...
    const epsilon = weighting.epsilonMargin > 0 && typeof params.epsilon === 'number'
      ? ScalarField.materialize(
          ScalarField.map(confidence!, c => params.epsilon as number + (1 - c) * weighting.epsilonMargin),
          input.width,
          input.height
        )
      : params.epsilon;
 ...
 */
export function blendByConfidence(a: ChannelImage, b: ChannelImage, confidence: ScalarField): ChannelImage {
  return ScalarField.materialize(
    ScalarField.blend(ScalarField.fromChannelImage(a), ScalarField.fromChannelImage(b), confidence),
    a.width,
    a.height
  );
}
