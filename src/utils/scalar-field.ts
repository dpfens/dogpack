...
// `ScalarField` here is declared as a genuinely local interface (extending
// the one from interfaces/base.ts) rather than merged from an `import
// type` of the same name. The import-type+const merge trick looks
// equivalent under a plain `tsc` run, but isn't reliably accepted once
// `verbatimModuleSyntax`/`isolatedModules` are on (each file has to be
// transpilable in isolation, without cross-checking that a later `const`
// declaration completes the merge) -- this local-interface form sidesteps
// that entirely, since nothing about it depends on the import being
// re-exported.
...
export const ScalarField = {
  ...
  fromChannelImage(img: ChannelImage): ScalarField {
    return { sample: (i: number) => img.data[i] };
  },

  /** Pointwise transform. */
  map(field: ScalarField, fn: (value: number) => number): ScalarField {
    return { sample: (i: number) => fn(field.sample(i)) };
  },
  ...
  scale(field: ScalarField, factor: ScalarField): ScalarField {
    return { sample: (i: number) => field.sample(i) * factor.sample(i) };
  },
  ...
  materialize(field: ScalarField, width: number, height: number): ChannelImage {
    ...
    for (let i = 0; i < size; i++) out.data[i] = field.sample(i);
    return out;
  },
};
 ...
 */
export function normalizedMagnitudeField(flow: FlowField): ScalarField {
  ...
  return {
    sample: (i: number) => flow.getMagnitude(i % width, (i / width) | 0) / max,
  };
}

/** Bridge a FlowField's per-pixel anisotropy into a ScalarField. Already
 *  in [0,1], no normalization needed. */
export function anisotropyField(flow: FlowField): ScalarField {
  const { width } = flow;
  return {
    sample: (i: number) => flow.getAnisotropy(i % width, (i / width) | 0),
  };
}
 ...
 */
export function flowConfidenceField(flow: FlowField): ScalarField {
  return ScalarField.scale(anisotropyField(flow), normalizedMagnitudeField(flow));
}