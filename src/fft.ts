...
export interface FFTOptions {
  maxSize?: number; // Maximum input size to pre-allocate for
}
...
export class WebGPUFFT {
  ...
  private computeBuffer: GPUBuffer | null = null;
   ...
   */
  private allocateBuffers(size: number): void {
   ...
   */
  async fft(data: Float32Array): Promise<Float32Array> {
    ...
    const result = new Float32Array(
      this.stagingBuffer!.getMappedRange().slice(0, n * 8)
    );
   ...
   * Compute inverse FFT
   */
  async ifft(data: Float32Array): Promise<Float32Array> {
    if (!this.device || !this.queue || !this.pipeline || !this.bindGroup) {
      throw new Error("FFT not initialized. Call init() first.");
    }
