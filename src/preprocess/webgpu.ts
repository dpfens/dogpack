...
export class GPUMedianFilter extends BaseWebGPUStrategy implements Preprocessor {
  ...
  constructor(config: Partial<MedianFilterConfig> = {}) {
    ...
    if (this.config.radius > 6) {
      console.warn(
        `GPUMedianFilter: radius=${this.config.radius} means a per-pixel ` +
...
export const GPUPreprocessingPresets = {
  ...
  light: (input: ChannelImage): Promise<ChannelImage> =>
    new GPUBilateralFilter({ sigmaSpatial: 2, sigmaRange: 0.08 }).process(input),
  ...
  standard: (input: ChannelImage): Promise<ChannelImage> =>
    new GPUBilateralFilter({ sigmaSpatial: 4, sigmaRange: 0.1 }).process(input),
