...
import type { BlurStrategy, ChannelImage, LocalBaselineOptions, MagnitudeAdaptiveOptions, ToneAdaptiveAutoOptions, ToneAdaptiveOptions, VarianceAdaptiveOptions } from '../../interfaces/base.js';
import { createChannelImage } from '../../utils/index.js';
import { IsotropicBlur } from '../../blur/isotropic.js';

async function resolveBlur(provided?: BlurStrategy): Promise<{ blur: BlurStrategy; owns: boolean }> {
  if (provided) return { blur: provided, owns: false };
  return { blur: await IsotropicBlur.create({ kernelSizeMultiplier: 4 }), owns: true };
}

/** value(x) = low + (high - low) * tanh(s * localTone(x)) */
export async function toneAdaptiveEstimate(input: ChannelImage, options: ToneAdaptiveOptions): Promise<ChannelImage> {
  ...
  try {
    ...
    if (owns) blur.dispose();
  }
}

/** Convenience: derive low/high from a center + spread instead of picking both by hand. */
export async function toneAdaptiveEstimateAuto(input: ChannelImage, options: ToneAdaptiveAutoOptions): Promise<ChannelImage> {
...
export async function localBaselineEstimate(input: ChannelImage, options: LocalBaselineOptions): Promise<ChannelImage> {
...
function normalizeToUnit(field: ChannelImage, gamma: number): ChannelImage {
  let max = 0;
  for (let i = 0; i < field.data.length; i++) if (field.data[i] > max) max = field.data[i];
...
function gradientMagnitude(input: ChannelImage): ChannelImage {
...
export async function magnitudeAdaptiveEstimate(input: ChannelImage, options: MagnitudeAdaptiveOptions): Promise<ChannelImage> {
...
export async function varianceAdaptiveEstimate(input: ChannelImage, options: VarianceAdaptiveOptions): Promise<ChannelImage> {
