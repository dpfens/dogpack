...
import type { ChannelImage, MagnitudeAdaptiveOptions, ToneAdaptiveAutoOptions, ToneAdaptiveOptions } from '../../interfaces/base.js';
import * as shared from './shared.js';

/** Recommended default. p(x) = pWeak + (pStrong - pWeak) * normalizedGradientMagnitude(x)^gamma */
export async function magnitudeAdaptiveEstimate(
  input: ChannelImage,
  options: Omit<MagnitudeAdaptiveOptions, 'low' | 'high'> & { pWeak: number; pStrong: number }
...
export async function toneAdaptiveEstimate(
  input: ChannelImage,
  options: Omit<ToneAdaptiveOptions, 'low' | 'high'> & { pDark: number; pLight: number }
): Promise<ChannelImage> {
...
export async function toneAdaptiveEstimateAuto(
  input: ChannelImage,
  options: ToneAdaptiveAutoOptions
): Promise<ChannelImage> {
