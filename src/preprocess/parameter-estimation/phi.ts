...
export async function varianceAdaptiveEstimate(
  input: ChannelImage,
  options: Omit<VarianceAdaptiveOptions, 'low' | 'high'> & { phiSoft: number; phiHard: number }
...
export async function toneAdaptiveEstimate(
  input: ChannelImage,
  options: Omit<ToneAdaptiveOptions, 'low' | 'high'> & { phiDark: number; phiLight: number }
): Promise<ChannelImage> {
