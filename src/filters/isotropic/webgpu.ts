 ...
 */
export class WebGPUIsotropicFilter extends BaseWebGPUStrategy implements EdgeAwareFilterCore<IsotropicBlurConfig> {
  ...
  async apply(input: ChannelImage, config: IsotropicBlurConfig): Promise<ChannelImage> {
    ...
    const kernelSize = Math.min(
      cfg.maxKernelSize,
      Math.max(3, Math.floor(sigma * cfg.kernelSizeMultiplier) | 1)
    );
