...
exports.computeStructureTensorMaps = computeStructureTensorMaps;
...
function boxBlur(data, width, height, radius) {
    ...
    for (let y = 0; y < height; y++) {
        ...
        let sum = 0;
        for (let j = 0; j < w; j++)
            sum += data[row + Math.max(0, Math.min(width - 1, j - radius))];
        h[row] = sum / w;
    ...
    for (let x = 0; x < width; x++) {
        let sum = 0;
        for (let j = 0; j < w; j++)
            sum += h[Math.max(0, Math.min(height - 1, j - radius)) * width + x];
        out[x] = sum / w;
...
function computeStructureTensorMaps(input, smoothingRadius = 2) {
    ...
    const magnitude = { data: new Float32Array(data.length), width, height };
    const anisotropy = { data: new Float32Array(data.length), width, height };
    for (let i = 0; i < data.length; i++) {
        const trace = e[i] + g[i];
...
//# sourceMappingURL=structure-tensor.js.map