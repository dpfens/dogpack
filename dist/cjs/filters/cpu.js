...
exports.EdgeAwareFilterPresets = exports.LocalVarianceFilter = exports.Quantizer = exports.ContrastEnhancer = exports.GaussianBlur = exports.KuwaharaFilter = exports.MedianFilter = exports.BilateralFilter = void 0;
 ...
 */
class BilateralFilter extends base_js_2.BaseCPUStrategy {
    async apply(input, config) {
        const cfg = { ...base_js_1.DEFAULT_BILATERAL_CONFIG, ...config };
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                const centerValue = (0, image_js_1.getPixel)(input, x, y);
                let sum = 0;
                let weightSum = 0;
                let idx = 0;
                for (let dy = -radius; dy <= radius; dy++) {
                    for (let dx = -radius; dx <= radius; dx++) {
                        const nx = x + dx;
                        const ny = y + dy;
                        const neighborValue = (0, image_js_1.getPixel)(input, nx, ny);
 ...
 */
class MedianFilter extends base_js_2.BaseCPUStrategy {
    async apply(input, config) {
        const cfg = { ...base_js_1.DEFAULT_MEDIAN_CONFIG, ...config };
        const { width, height } = input;
        const output = (0, image_js_1.createChannelImage)(width, height);
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                for (let dy = -radius; dy <= radius; dy++) {
                    for (let dx = -radius; dx <= radius; dx++) {
                        values[idx++] = (0, image_js_1.getPixel)(input, x + dx, y + dy);
                    }
                }
 ...
 */
class KuwaharaFilter extends base_js_2.BaseCPUStrategy {
    async apply(input, config) {
        const cfg = { ...base_js_1.DEFAULT_KUWAHARA_CONFIG, ...config };
        const { width, height } = input;
        const output = (0, image_js_1.createChannelImage)(width, height);
        const r = cfg.radius;
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                let bestMean = (0, image_js_1.getPixel)(input, x, y);
                for (const q of quadrants) {
                    ...
                    for (let dy = q.startY; dy <= q.endY; dy++) {
                        for (let dx = q.startX; dx <= q.endX; dx++) {
                            const val = (0, image_js_1.getPixel)(input, x + dx, y + dy);
                            sum += val;
                            sumSq += val * val;
                            count++;
                        }
 ...
 */
class GaussianBlur extends base_js_2.BaseCPUStrategy {
    async apply(input, config) {
        const { width, height } = input;
        const sigma = config.sigma ?? base_js_1.DEFAULT_GAUSSIAN_CONFIG.sigma;
        if (sigma < 0.1) {
        ...
        // Horizontal pass
        const temp = (0, image_js_1.createChannelImage)(width, height);
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                let val = 0;
                for (let k = 0; k < kernelSize; k++) {
                    val += (0, image_js_1.getPixel)(input, x + k - radius, y) * kernel[k];
                }
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                let val = 0;
                for (let k = 0; k < kernelSize; k++) {
                    val += (0, image_js_1.getPixel)(temp, x, y + k - radius) * kernel[k];
                }
 ...
 */
class ContrastEnhancer extends base_js_2.BaseCPUStrategy {
    async apply(input, config) {
        const cfg = { ...base_js_1.DEFAULT_CONTRAST_ENHANCEMENT_CONFIG, ...config };
 ...
 */
class Quantizer extends base_js_2.BaseCPUStrategy {
    async apply(input, config) {
        const cfg = { ...base_js_1.DEFAULT_QUANTIZER_CONFIG, ...config };
        const { width, height, data } = input;
        const output = (0, image_js_1.createChannelImage)(width, height);
        const size = width * height;
...
exports.LocalVarianceFilter = LocalVarianceFilter;
/**
 * Preset preprocessing pipelines for common use cases
 */
exports.EdgeAwareFilterPresets = {
    /**
     * Light preprocessing - minimal smoothing
     * Good for: Clean studio photos, illustrations
     */
    light: async (input) => {
        return await new BilateralFilter().apply(input, { sigmaSpatial: 2, sigmaRange: 0.08 });
    },
    /**
    ...
    standard: async (input) => {
        return new BilateralFilter().apply(input, { sigmaSpatial: 4, sigmaRange: 0.1 });
    },
    /**
     ...
     */
    heavy: async (input) => {
        let result = await new BilateralFilter().apply(input, { sigmaSpatial: 5, sigmaRange: 0.12 });
        result = await new BilateralFilter().apply(result, { sigmaSpatial: 3, sigmaRange: 0.1 });
        return result;
    },
    /**
    ...
    artistic: async (input) => {
        let result = await new KuwaharaFilter().apply(input, { radius: 4 });
        result = await new BilateralFilter().apply(result, { sigmaSpatial: 2, sigmaRange: 0.08 });
        return result;
    },
    /**
    ...
    nature: async (input) => {
        ...
        let result = await new BilateralFilter().apply(input, { sigmaSpatial: 6, sigmaRange: 0.15 });
        // Second pass: lighter bilateral to clean up
        result = await new BilateralFilter().apply(result, { sigmaSpatial: 3, sigmaRange: 0.08 });
        return result;
    },
};
