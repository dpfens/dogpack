...
exports.magnitudeAdaptiveEstimate = magnitudeAdaptiveEstimate;
exports.varianceAdaptiveEstimate = varianceAdaptiveEstimate;
const index_js_1 = require("../../utils/index.js");
const isotropic_js_1 = require("../../blur/isotropic.js");
async function resolveBlur(provided) {
    if (provided)
        return { blur: provided, owns: false };
    return { blur: await isotropic_js_1.IsotropicBlur.create({ kernelSizeMultiplier: 4 }), owns: true };
}
...
async function localBaselineEstimate(input, options) {
    ...
    try {
        ...
        const output = (0, index_js_1.createChannelImage)(input.width, input.height);
        if (contrastMargin > 0) {
            const squared = (0, index_js_1.createChannelImage)(input.width, input.height);
...
function normalizeToUnit(field, gamma) {
    ...
    for (let i = 0; i < field.data.length; i++)
        if (field.data[i] > max)
            max = field.data[i];
    const output = (0, index_js_1.createChannelImage)(field.width, field.height);
    if (max <= 0)
        return output;
    for (let i = 0; i < output.data.length; i++) {
...
async function varianceAdaptiveEstimate(input, options) {
    ...
    try {
        ...
        for (let i = 0; i < input.data.length; i++)
            squared.data[i] = input.data[i] * input.data[i];
        const meanSquared = await blur.blur(squared, sigma);
        const variance = (0, index_js_1.createChannelImage)(input.width, input.height);
