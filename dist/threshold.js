 ...
 */
export class AdaptiveThresholdStrategy {
    threshold(sharpened, config) {
     ...
     */
    computeLocalContrast(image, radius) {
 ...
 */
export class BilateralThresholdStrategy {
    threshold(sharpened, config) {
     ...
     */
    bilateralFilter(image, radius, sigmaIntensity) {
        const output = createChannelImage(image.width, image.height);
        const { width, height } = image;
